...
const TrendingItem = ({ activeItem, item }) => {
  if (!item) return null; // ✅ Захист від undefined
  ...
  return (
    <Animatable.View
      className="mr-5"
      animation={activeItem && activeItem === item.$id ? zoomIn : zoomOut} // ✅ Уникає помилки
      duration={500}
    >
      {play ? (
        <Video
...
const Trending = ({ posts }) => {
  ...
  return (
    <FlatList
      data={posts || []}
      horizontal
      keyExtractor={(item, index) => (item?.$id ? item.$id.toString() : index.toString())} // ✅ Перевірка
